...
class flex_code():
    ...
    eyx=0    eyy=1    eyz=0\n\
    ezx=0    ezy=0    ezz=1\n\
    eps=tensor((exx,exy,exz),(eyx,eyy,eyz),(ezx,ezy,ezz))\n\
    D=dot(eps,E_v)\n".format(self.Ps,self.cubes_num[2]))
            f.write("EQUATIONS \n")
            f.write("uv: e0*div(D)=0 \n")
            f.write("EXTRUSION\n")
            ...
            for x in range (1,cubes_num[0]+1):
                for y in range (1,cubes_num[1]+1):
                    ...
                    for z in range (1,cubes_num[2]+1):
                        ...
                        f.write("           Ex=VOL_INTEGRAL(xcomp(E_v),'{}.{}','{}-{}')/v".format(x,y,z,z+1))
                        f.write("           Ey=VOL_INTEGRAL(ycomp(E_v),'{}.{}','{}-{}')/v".format(x,y,z,z+1))
                        f.write("           Ez=VOL_INTEGRAL(zcomp(E_v),'{}.{}','{}-{}')/v\n".format(x,y,z,z+1))
                        if z!=cubes_num[2]:
            ...
            f.write("table(magnitude(E_v)) format '#1' points=100 file='Em.txt'\n")
    ...
    eyx=0    eyy=1    eyz=0\n\
    ezx=0    ezy=0    ezz=1\n\
    sw=USTEP(magnitude(cross(vector(Ex,Ey,Ez),P)) /Ea2-dot(vector(Ex,Ey,Ez),P)/Ea)*USTEP(t-1e-7)\n\
    a= tensor((1,0,0),(0,0,1),(0,-1,0))*sw+tensor((1,0,0),(0,1,0),(0,0,1))*(1-sw)\n\
    eps0=tensor((exx,exy,exz),(eyx,eyy,eyz),(ezx,ezy,ezz))\n\
    eps=DOT(a,eps0)\n\
            ...
            for x in range (1,cubes_num[0]+1):
                for y in range (1,cubes_num[1]+1):
                    ...
                    for z in range (1,cubes_num[2]+1):
                        ...
                        f.write("           eyx={}    eyy={}    eyz={}\n".format(epsilon[z-1][y-1][x-1][1][0],epsilon[z-1][y-1][x-1][1][1],epsilon[z-1][y-1][x-1][1][2]))
                        f.write("           ezx={}    ezy={}    ezz={}\n".format(epsilon[z-1][y-1][x-1][2][0],epsilon[z-1][y-1][x-1][2][1],epsilon[z-1][y-1][x-1][2][2]))
                        f.write("           Ex=VOL_INTEGRAL(xcomp(E),'{}.{}','{}-{}')/v".format(x,y,z,z+1))
                        f.write("           Ey=VOL_INTEGRAL(ycomp(E),'{}.{}','{}-{}')/v".format(x,y,z,z+1))
